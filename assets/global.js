...
function pauseAllMedia() {
  ...
  document.querySelectorAll('video').forEach((video) => {
    video.pause();
    let playButton = video.closest('.media').querySelector('.video-play-btn');
...
class HeaderDrawer extends MenuDrawer {
  constructor() {
    super();
    this.openButtons = document.querySelectorAll('[aria-controls="cart-drawer"]');
    this.openButtons.forEach((button) => button.addEventListener('click', this.openMenuDrawer.bind(this)));
  }
...
class ComponentVariantSubscriptionPrices extends HTMLElement {
  constructor() {
    super();
  }
...
const mobileMenuTabs = () => {
  const buttons = document.querySelectorAll('.menu-drawer__main-item');
  const panels = document.querySelectorAll('.menu-drawer__panel');
  ...
  buttons.forEach((button) => {
    button.addEventListener('click', () => {
      ...
      buttons.forEach((button) => {
        if (button.getAttribute('aria-controls') === target) {
...
class HorizontalScroller extends HTMLElement {
  constructor() {
    ...
    this.scrollStep = this.content.firstElementChild.offsetWidth + this.gap;
...
class DropdownSelect extends HTMLElement {
  constructor() {
    super();
    this.button = this.querySelector('div[role="button"]');
  ...
  selectOption(event) {
    ...
    this.realSelect.value = event.currentTarget.getAttribute('data-value');
  ...
  toggleListbox(event) {
    ...
    this.button.setAttribute('aria-expanded', this.hasAttribute('open') ? 'true' : 'false');
  }
...
const setCookie = (name, value, days) => {
  document.cookie = `${name}=${value}; path=/; expires=${new Date(days * 1000 * 60 * 60 * 24)}`;
}
